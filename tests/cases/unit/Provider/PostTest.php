...
class PostTest extends ProviderTestCase
{
    public function testNoPropertiesCreation()
    {
        ...
        static::assertContains($post->post_status, ['publish', 'draft', 'future']);
        static::assertContains($post->comment_status, ['open', 'closed']);
        static::assertContains($post->ping_status, ['open', 'closed']);
        ...
        static::assertContains($post->post_type, ['post', 'page']);
        static::assertSame('', $post->post_mime_type);
        static::assertSame('0', $post->comment_count);
        static::assertContains($post->filter, ['raw', null]);
